...
void HistTool::histoLoop(std::string year , vector<string> files, string dir, string tree_name , string var_name, vector<float> OSSS, bool runPDF,string Sys = "") {
    ...
    for (auto ifile : files) {
        ...
        tree->SetBranchAddress("lowPurity",&lowPurity);
        ...
        for (auto i = 0; i < tree->GetEntries(); i++) {
            ...
            if (name.find("000")!=string::npos){
            LumiWeight = LumiWeight * .001;};
            ...
            //            ################################################################################
// apply the cuts here
            cout<<observable <<"  "<<weight<<"\n";
    
            if (HH4tau_NN_output<0.7) { // final analysis
